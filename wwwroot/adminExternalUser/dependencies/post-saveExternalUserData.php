<?php
	...
	if (($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "SUPER" && (!isset($_POST['lastName']) || !isset($_POST['firstName']) || !isset($_POST['attId']) || !isset($_POST['loginId']) || !isset($_POST['emailAddress']) || !isset($_POST['externalUserStatus']))) && ($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] == "SUPER" && (!isset($_POST['lastName']) || !isset($_POST['firstName']) || !isset($_POST['emailAddress']) || !isset($_POST['externalUserStatus'])))) {
		$type = "error";
	...
	$externalUserStatus = $_POST['externalUserStatus'];
	...
	if ($count < 1) {
		...
		if ($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ADD" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ALL" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "SUPER") {
			$type = "error";
			$message = "YOU DO NOT HAVE PERMISSIONS TO ADD EXTERNAL USERS";
			$field = "";
			goto echoResponse;
		}
		...
		if ($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "EDIT" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ALL" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "SUPER") {
			$type = "error";
			$message = "YOU DO NOT HAVE PERMISSIONS TO EDIT EXTERNAL USERS";
			$field = "";
			goto echoResponse;
		}
	...
	// Check if GUID exists in lookupExternalUserInfo
	$sql = "SELECT COUNT(*) AS COUNT FROM lookupExternalUserInfo WHERE GUID = '$GUID'";
	$results = $Database->Query($sql);
	$myrow = odbc_fetch_array($results);
	$count = $myrow['COUNT'];
	...
	if ($count < 1) {
		if ($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ADD" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ALL" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "SUPER") {
			$type = "error";
			$message = "YOU DO NOT HAVE PERMISSIONS TO ADD EXTERNAL_USERS";
			$field = "";
			goto echoResponse;
		}
		// There was no record found for the provided GUID in lookupExternalUserInfo, insert the record
		$sql = "INSERT  INTO lookupExternalUserInfo (GUID, externalUserStatus) VALUES ('$GUID', '$externalUserStatus')";
		$results = $Database->Query($sql);
	} else {
		// There was a record found for the provided GUID in lookupExternalUserInfo, update the record
		// Before we begin the update, capture the current status of externalUserStatus so we can evaluate if its changing
		if ($userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "EDIT" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "ALL" && $userInfo['userPermissions']['tools']['ADMINEXTERNALUSER'] != "SUPER") {
			$type = "error";
			$message = "YOU DO NOT HAVE PERMISSIONS TO EDIT EXTERNAL USERS";
			$field = "";
		...
		$sql = "SELECT externalUserStatus FROM lookupExternalUserInfo WHERE GUID='$GUID'";
		$results = $Database->Query($sql);
		$myrow = odbc_fetch_array($results);
		if ($externalUserStatus != $myrow['externalUserStatus']) {
			// ExternalUser Status was changed, flag this for being checked for perissios needing to be modified
			$updatePermissions = true;
		}
		$sql = "UPDATE lookupExternalUserInfo SET";
		$sql .= " externalUserStatus = '$externalUserStatus'";
		$sql .= " WHERE GUID = '$GUID'";
		$results = $Database->Query($sql);
	}
	...
	if ($updatePermissions) {
		...
		// if a externalUserCount or userCount > 0 then an Active Record exists for one of them, do nothing.
		if ($agentCount == 0 && $userCount == 0) { 
			if ($externalUserCount == 1) {
				// Only externalUser login exists, set to default externalUser permission 
				//***************SET THIS SQL PROPERLY TO GRANT DEFAULT EXTERNAL USERS **************************
//				$sql = "UPDATE lookupPermissions SET";
...
//				$results = $Database->Query($sql);
				$type = "success";
				if ($_POST['GUID'] == "") {
					$message = "EXTERNAL USER RECORD SUCCESSFULLY ADDED. [" . $lastName . ", " . $firstName . " (" . strtoupper($attId) . ")]";
				} else {
					$type = "success";
					$message = "EXTERNAL USER RECORD SUCCESSFULLY UPDATED. [" . $lastName . ", " . $firstName . " (" . strtoupper($attId) . ")]";
				}
...
//				$results = $Database->Query($sql);
				$type = "success";
				if ($_POST['GUID'] == "") {
					$message = "<div>EXTERNAL USER RECORD SUCCESSFULLY ADDED. [" . $lastName . ", " . $firstName . " (" . strtoupper($attId) . ")]";
				} else {
					$message = "<div>EXTERNAL USER RECORD SUCCESSSFULLY UPDATED. [" . $lastName . ", " . $firstName . " (" . strtoupper($attId) . ")]";
				}
		...
		if ($_POST['GUID'] == "") {
			$type = "success";
			$message = "EXTERNAL USER RECORD SUCCESSFULLY ADDED [" . $lastName . ", " . $firstName . " (" . $attId . ")]";
		} else {
			$type = "success";
			$message = "EXTERNAL USER RECORD SUCCESSFULLY UPDATED [" . $lastName . ", " . $firstName . " (" . strtoupper($attId) . ")]";
			$field = "";
