<?php
	...
	if (!isset($userInfo['userPermissions']['tools']['ADMINSUPERVISOR']) || $getGUID == "") {
		...
		echo "		permissions required: ADMINSUPERVISOR: [ADD], [EDIT], [VIEW], [ALL]<br />\n";
	...
	if ($userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "ADD" || $userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "ALL") {
		$classHideAddButton = "";
	}
	// Set variable to hide the edit selections if you don't have EDIT, VIEW, or ALL access
	$classHideEditSelection = "class='hidden'";
	if ($userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "EDIT" || $userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "VIEW" || $userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "ALL") {
		$classHideEditSelection = "";
	}

	// Set variable to determine actions available from select list
	$options = "EDIT/VIEW:";
	if ($userInfo['userPermissions']['tools']['ADMINSUPERVISOR'] == "VIEW") {
		$options = "VIEW:";
	}
?>
...
<html>
	<head>
		<title>SUPERVISOR ADMINISTRATION</title>
		<!-- Add Stylesheets -->
...
?>
		<div class='center'><h2>SUPERVISOR ADMINISTRATION</h2></div>
		<div id='formLoad'></div>
		...
		<div id='formWrapper'>
			<table style='border-collapse: collapse;' align='center' style='padding: 5px;'>
				<tr>
					<td class='labelAbove' <?php echo $classHideEditSelection ?>>SELECT SUPERVISOR TO <?php echo $options ?>&nbsp;&nbsp;</td>
				...
				<tr>
					...
					<td>
						<div <?php echo $classHideAddButton ?>'>&nbsp;&nbsp;<input type='button' name='btnAddSupervisor' id='btnAddSupervisor' value='ADD SUPERVISOR' /></div>
					</td>
					<td <?php echo $classHideEditSelection ?>>
						<div>
			...
			<form name='frmSupervisorAdministration' id='frmSupervisorAdministration'>
				<div id='divSupervisorAdminBody' name='divSupervisorAdminBody'>
					<table class='bordered centered spaced filled'>
						...
						<tr>
							...
							<td class='data GUID'>
								<input class='tooltipHover tooltipFocus' type='text' READONLY name='GUID' id='GUID' size='75' maxlength='36' tooltipText='The supervisor Database Record identifier. This field will eventually be removed.' placeholder='AUTOMATICALLY GENERATED ON SAVE' />
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class='data loginId'>
								<input class='changekey tooltipHover tooltipFocus forceLowerCase' type='text' name='loginId' id='loginId' size='75' maxlength='25' tooltipText='Usually the same as the AT&T ID, but if supervisor logs in with a different login, set it here.' placeholder = 'WINDOWS LOGIN' required='required' />
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td class='data label'>SUPERVISOR STATUS:</td>
							<td>
								<div id="buttonSupervisorStatus" class='tooltipHover' tooltipText='Inactive supervisors do not display on drop down lists by default, but data remains.'>
									<button class="buttonGroup" id="ACTIVE">ACTIVE</button>
									<button class="buttonGroup" id="INACTIVE">INACTIVE</button>
								</div>
								<input type='hidden' id='supervisorStatus' name='supervisorStatus' />
							</td>
						</tr>
						<tr>
