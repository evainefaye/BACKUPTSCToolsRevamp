...
// The var that stores the error (if any)
	var $Error = "";
		...
		function MyDatabase() {
			...
			$this->DBPass = "Macross12";
			$this->DBName = "tsc_tools";
			$this->DBHost = ".\SQLExpress";
		}

		// Connect to the database
		function Connect() {
			$this->db = odbc_connect("Driver={SQL Server};Server=$this->DBHost;Database=$this->DBName;MARS_Connection=yes", $this->DBUser, $this->DBPass) or die("DATABASE CONNECTION ERROR: ". odbc_errormsg());
		...
		function Disconnect() {
			$this->db = odbc_connect("Driver={SQL Server};Server=$this->DBHost;Database=$this->DBName;MARS_Connection=yes", $this->DBUser, $this->DBPass)  or die("DATABASE ERROR:". odbc_errormsg());
			odbc_close($this->db);
		}
