<?php
	...
	if (isset($_POST['ExternalUserGUID'])) {
		$ExternalUserGUID = $_POST['ExternalUserGUID'];
		if ($ExternalUserGUID != "") {
			$ExternalUserGUID=" OR lookupExternalUserInfo.GUID='" . $ExternalUserGUID . "'";
		} else {
			$ExternalUserGUID = "";
		}
	} else {
	...
	// Get list of active ExternalUsers from lookupExternalUserInfo, generate JSON with GUID and LastName
	$sql = "SELECT * FROM lookupExternalUserInfo LEFT JOIN lookupPermissions ON lookupExternalUserInfo.GUID=lookupPermissions.GUID WHERE lookupPermissions.GUID IS NOT NULL ORDER BY lastName, firstName ASC";
	...
	while ($myrow = odbc_fetch_array($results)) {
		...
		$ExternalUserStatus = $myrow['ExternalUserStatus'];
		$selExternalUserId[] = array(
			'GUID' => $guid,
			'fullName' => $fullName,
			'attId' => $attId,
			'displayName' => $displayName,
			'ExternalUserStatus' => $ExternalUserStatus
		);
	}
	echo json_encode($selExternalUserId);
